...
### 변경 (Changed) — 코드 변경 없음, 문서/운영 가이드만 갱신
- 📘 사용설명서.md §7.6 **"사내 매핑 드라이브가 동작하지 않을 때 (NPFS DRM 환경)"** 신설
  - PowerShell 1줄 진단 명령
  - 우회 방법: DRM 미적용 별도 SMB 서버(UNC 직접 접속) 사용
- 🧠 SKILL 19 (서버 폴더 접근) v1.2 보완 (.agent-master)
  - "이 패턴이 동작하지 않는 환경 — NPFS DRM 사각지대" 신규 섹션
  - DB_Manager 케이스 사례 기록
  - 영문/한글 동일 보완

### 발견 사항 (Findings)
- 사내 보안망 매핑 드라이브 `Z:` (FileSystem=NPFS, DriveType=3) 환경에서 메모장 외 모든 사용자 모드 코드(python.exe / cmd.exe / powershell.exe / 시스템 COM 객체)가 read 거부
- 10가지 우회 시도(MSXML/ADODB/FSO/cscript 등) 모두 실패 — **커널 미니필터가 호출 프로세스 EXE 화이트리스트로 차단**
- **유일한 해결책은 DRM 미적용 별도 SMB 서버 사용** (`\\10.60.20.11\...`) — 코드 수정 불필요
...
### 수정된 버그 (Fixed)
- 🐛 **사내 매핑 드라이브(`Z:\...`)에서 XML 파일이 모두 `Errno 13: Permission denied`로 거부되던 버그 수정**
  - 원인: `xml_parser._open_xml()`이 모든 경로에 Windows long-path 접두사(`\\?\`)를 자동 추가
  - 한글/공백/특수문자가 섞인 경로(약 200자)에서 prefix가 SMB 권한 토큰 검증 경로를 변경하여 권한 거부 유발
  - Windows 탐색기로는 정상 열림, `iterdir()`로 디렉터리 listing은 성공하나 `open()`만 실패하는 패턴
  - 수정: `_open_xml`을 단순 `open(str(path), 'rb')`로 변경 (Sliding Stage OPM 프로젝트와 동일 패턴)
  - 결과: 사내 매핑 드라이브에서 로컬 복사 없이 직접 DB 로딩 가능

### 변경 (Changed)
- 🔧 SKILL 19 (서버 폴더 접근) v1.1로 보완 — `\\?\` 접두사 자동 추가 안티패턴 명시
  - 영문/한글 SKILL.md 양쪽에 anti-pattern 경고 박스 + 함정 항목 + 테스트 체크리스트 추가

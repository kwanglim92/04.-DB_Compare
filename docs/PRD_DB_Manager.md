...
**v1.0(2025-12-26) 최초 릴리즈 이후 4개월간 v1.5.1까지 진화**하여, 단순 검증 도구에서 **서버 기반 Spec 동기화 + 검색·임포트·자동업데이트·리포트 템플릿 + 출고 문서 자동기입(v1.6 진행)** 까지 포괄하는 QC 운영 도구로 자리잡았다. 다음 단계는 **v1.6 Industrial Checklist Auto-Fill**(고객 출고용 엑셀 자동 기입)으로, QC 워크플로우의 마지막 병목인 "수기 G열 채움 단계"를 제거한다.
...
| **출고 엑셀 G열 오기입율** | 수기 입력 (체감 5~10%) | 동일 | 동일 | **자동 기입 + Dry-run 검증으로 근사 0** |
...
### 2.1 제품 정의
Park Systems XE Service DB(XML 기반)를 파싱하여 사전 정의된 QC 프로파일과 비교하고, Excel 리포트를 자동 생성하며, 고객 출고용 Industrial Checklist 엑셀에 자동 기입하는 Windows 데스크톱 애플리케이션.
...
| **프로파일 관리** | Common Base 상속 구조, 듀얼 뷰(Table/Tree), 실시간 검색, CRUD | v1.0~v1.1 |
...
| **🆕 결과 필터** | Profile Viewer Segmented Button (All/Pass/Check/Fail) | **v1.4** |
...
| **🆕 Industrial Checklist Auto-Fill** | QC 결과를 고객 출고용 엑셀 G열에 자동 기입 (5단 매핑 캐스케이드) | **v1.6 (진행 중)** |
...
5. **출고 문서 무결성**: Industrial Checklist 엑셀 G열 자동 기입 + 백업·Dry-run으로 **수기 오기입 0건** (v1.6 목표).
...
- **기대**: 로드→실행→리포트→Auto-Fill의 원클릭 경험
...
- QC 엔지니어로서, **고객 출고용 엑셀 G열을 손으로 채우지 않고 자동 기입**받고 싶다 (🟡 v1.6 진행).
...
### 8.4 Must-Have (P0) — v1.6 (Industrial Checklist Auto-Fill) — 🟡 진행 중

> **현황 (2026-04-28 기준)**: Phase 1~6 코드 구현 완료, 서버 마이그레이션 완료(`checklist_mappings` 테이블), 단위·통합 테스트 36/36 통과. 빌드 검증 + 사용자 검증 단계 남음.
...
| R22 | G열 자동 기입 + 안전 가드 | 🟡 진행 중 | 백업 자동 생성, 금지 영역 화이트리스트, Dry-run 기본 ON, 셀 색상, `_AutoFill_Log` 시트 |
| R23 | 검수자 UI 다이얼로그 | 🟡 진행 중 | 통계 바, 트리뷰, 미매핑 강조, Dry-run/확정 액션 |
| R24 | 메타데이터 자동 채움 (Last 시트) | 🟡 진행 중 | L22~L43 매핑 (Model/SID/Date/엔지니어) |

### 8.5 Nice-to-Have (P1) — v1.7+

| ID | 요구사항 | 설명 |
|----|---------|------|
| R25 | 다국어(한/영) 지원 | 사내/해외법인 공동 사용 |
...
| R30 | 수동 매핑 → 즉시 학습 사전 저장 UI (v1.6 확장) | 다이얼로그에서 후보 선택 후 즉시 PG 업서트 |
...
### 9.2 v1.6 목표 (Auto-Fill)
```
QCComparator (qc_report)
    ↓
ChecklistMapper (5단 캐스케이드)  ←  SyncManager (checklist_mappings)
    ↓                                       ↓
ChecklistAutoFiller (G열 쓰기 + 안전 가드)   PostgreSQL checklist_mappings
    ↓
[고객 출고 엑셀] + 백업 + _AutoFill_Log 시트
```
...
| **v1.6.0** | **2026-Q2 (진행 중)** | **Industrial Checklist Auto-Fill** | 5단 캐스케이드 매핑, 학습 사전, G열 자동 기입, 안전 가드 |
| v1.7.0 | 2026-Q3 | 사용성·국제화 | 다국어, 아이템 상세, Auto-Fill 매핑 UI 보강 |
...
| 출고 엑셀 자동 기입 비율 (v1.6) | Auto-Fill로 채워진 셀 / 전체 G열 셀 | ≥ 90% (학습 사전 누적 후) | v1.6 출시 후 측정 |
...
| 6 | DB 포맷 변경(XE Service 업데이트) | 파서 호환성 | Parser 추상화, 회귀 테스트 정비 (현재 147 passed) |
| 7 | 사내 NPFS DRM 환경 매핑드라이브 차단 | 서버 폴더 미접근 | **별도 SMB 서버 사용** (UNC `\\10.60.20.11\...`) — v1.5.2에서 운영 가이드 추가 ✅ |
| 8 | v1.6 Auto-Fill 매핑 정확도 저조 | 자동화 효과 미흡 | 학습 사전 + Dry-run 기본 ON + 검수자 UI로 점진 학습 |
| 9 | v1.6 출고 엑셀 수식 손상 | 고객 전달 문서 손상 | G열만 쓰기 화이트리스트, 백업 자동 생성, openpyxl `data_only=False` 로딩 |
...
| Phase 3. Auto-Fill 엔진 | checklist_autofiller + 안전 가드 + Dry-run + _AutoFill_Log | ✅ 완료 | |
| Phase 4. 검수자 UI | checklist_autofill_dialog + main_window 진입점 | ✅ 완료 | Auto-Fill Checklist 버튼 |
| Phase 5. 메타데이터 채움 | _fill_metadata_sheet (Last 시트 L22~L43) | ✅ 완료 | |
| Phase 6. 테스트 | 단위/통합 테스트 36/36 통과 | ✅ 완료 | 픽스처 3 skipped |
| Phase 7. PyInstaller 빌드 검증 | EXE 동작 + rapidfuzz 검증 | ⚪ 대기 | 사용자 환경 별도 |
| Phase 8. 파일럿 + 정식 배포 | 2~3명 엔지니어 검증 → 전사 | ⚪ 대기 | |
...
| Q7 | v1.6 엑셀 M열 자동 채우기 기본값 (현재 OFF, 옵션) — **사내 보관본만 ON 권장** 정책 확정 필요 | QC 팀 리드 |
...
- **Auto-Fill**: 고객 출고용 Industrial Checklist 엑셀 G열에 QC 결과 자동 기입 (v1.6)
- **Dry-run**: 실제 파일 변경 없이 결과 미리보기만 생성하는 안전 모드
...
| **2.0** | **2026-04-28** | **v1.2~v1.5.1 완료 반영, v1.6 (Auto-Fill) 진행 상태 추가, 파일명 v1.1.0 제거 (영구 사용)** | **Levi.Beak** |

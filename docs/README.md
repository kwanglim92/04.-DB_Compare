...
3. (v1.6 진행) **Final Checklist QC** 버튼으로 checklist 최종 검수 → 승인 행만 내부 사본에 자동 기입
...
| **사용성** | DB 트리 검색(F3/F4 네비), Profile Viewer 상태 필터 + 검색 |
| **자동 업데이트** | PG `app_releases` 기반, 강제/일반 모드 |
| **리포트** | Excel 3-시트 (Summary / All / Failed) + 회사 브랜딩 템플릿 |
| **관리자 모드** | 영어 UI, 서버 설정 + Spec 관리 + 리포트 템플릿 통합, 프로필 삭제 재인증 |
| **Spec 관리** | 모듈 임포트, Add/Edit 팝업 검증, Type/Spec 미설정 항목 필터 |
| **출고 문서 최종 검수** (v1.6 진행) | Final Checklist QC, Preflight, 위험도, 승인 행만 내부 사본에 G열/DB_Key 기입 |
...
- Checklist 자동 기입은 원본을 수정하지 않고 내부 결과 사본만 생성
...
| 현재 버전 | v1.5.1 (+ v1.5.2 운영 가이드 보강, v1.6 Final Checklist QC 진행) |
| 다음 버전 | v1.6 — Final Checklist QC / Approved Auto-Fill |
...
- **CI/검증**: pytest 182 passed, 6 skipped (2026-04-29 현재 작업 브랜치 기준)
